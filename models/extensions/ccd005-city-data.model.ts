...
export class CCD005CityData {
  ...
  setCityActivationStatus(sender: Account, cityId: number, status: boolean) {
    return Tx.contractCall(this.name, "set-city-activation-status", [types.uint(cityId), types.bool(status)], sender.address);
  }
  ...
  getCityTreasuryId(cityId: number, treasuryName: string): ReadOnlyFn {
    return this.callReadOnlyFn("get-city-treasury-id", [types.uint(cityId), types.ascii(treasuryName)]);
  }

  getCityTreasuryName(cityId: number, treasuryId: number): ReadOnlyFn {
    return this.callReadOnlyFn("get-city-treasury-name", [types.uint(cityId), types.uint(treasuryId)]);
  }

  getCityTreasuryAddress(cityId: number, treasuryId: number): ReadOnlyFn {
    return this.callReadOnlyFn("get-city-treasury-address", [types.uint(cityId), types.uint(treasuryId)]);
  }

  getCityTokenContractNonce(cityId: number): ReadOnlyFn {
    return this.callReadOnlyFn("get-city-token-contract-nonce", [types.uint(cityId)]);
  }

  getCityTokenContractId(cityId: number, tokenAddress: string): ReadOnlyFn {
    return this.callReadOnlyFn("get-city-token-contract-id", [types.uint(cityId), types.principal(tokenAddress)]);
  }

  getCityTokenContractAddress(cityId: number, tokenId: number): ReadOnlyFn {
    return this.callReadOnlyFn("get-city-token-contract-address", [types.uint(cityId), types.uint(tokenId)]);
  }
