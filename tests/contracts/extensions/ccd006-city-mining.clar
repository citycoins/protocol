...
(define-private (mint-coinbase (cityName (string-ascii 10)) (cityId uint) (recipient principal) (blockHeight uint))
  (let
    ...
    (and (is-eq cityName "mia")
      (try! (as-contract (contract-call? .test-ccext-governance-token-mia mint amount recipient)))
    )
    (and (is-eq cityName "nyc")
      (try! (as-contract (contract-call? .test-ccext-governance-token-nyc mint amount recipient)))
    )
    (ok true)
  )
)
