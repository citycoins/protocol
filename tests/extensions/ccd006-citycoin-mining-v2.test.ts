...
import { CCD006CityMiningV2 } from "../../models/extensions/ccd006-citycoin-mining-v2.model.ts";
...
const twoMinersMine = (user1: Account, user2: Account, ccd006CityMining: CCD006CityMining, chain: Chain, sender: Account): any => {
  ...
  const miningClaimBlock = chain.mineBlock([ccd006CityMining.claimMiningReward(user1, mia.cityName, claimHeight), ccd006CityMining.claimMiningReward(user2, mia.cityName, claimHeight)]);

  miningBlock.receipts[0].events.expectSTXTransferEvent(10, user1.address, `${sender.address}.${mia.treasuryV1Contract}`);
  miningBlock.receipts[1].events.expectSTXTransferEvent(10, user2.address, `${sender.address}.${mia.treasuryV1Contract}`);
  let winner = 0;
  let coinbase = 0;
  if (miningClaimBlock.receipts[0].result === "(ok true)") {
    //console.log("======== USER 1 WINS =========================")
    ccd006CityMining.getBlockWinner(mia.cityId, claimHeight).result.expectSome().expectUint(1);
    coinbase = Number(ccd006CityMining.getCoinbaseAmount(mia.cityId, claimHeight).result.substring(1));
    ...
    */
    ...
    ccd006CityMining.getBlockWinner(mia.cityId, claimHeight).result.expectSome().expectUint(2);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // act
    const entries = [10, 10];
    const { receipts } = chain.mineBlock([ccd006CityMining.mine(sender, mia.cityName, entries)]);
...
Clarinet.test({
  name: "ccd006-citycoin-mining-v2: mine() fails if city details are not set",
  fn(chain: Chain, accounts: Map<string, Account>) {
    // arrange
    const sender = accounts.get("deployer")!;
    const ccd006CityMining = new CCD006CityMining(chain, sender, "ccd006-citycoin-mining");
...
Clarinet.test({
  name: "ccd006-citycoin-mining-v2: mine() fails if city treasury is not set",
  fn(chain: Chain, accounts: Map<string, Account>) {
    // arrange
    const sender = accounts.get("deployer")!;
    const ccd006CityMining = new CCD006CityMining(chain, sender, "ccd006-citycoin-mining");
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block.receipts[0].events.expectSTXTransferEvent(10, user.address, `${sender.address}.${mia.treasuryV1Contract}`);
    const expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(firstBlock)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(userId)}}`;
    //console.log(block.receipts[0].events[1].contract_event.value)
    //ccd006CityMining.isBlockWinner(mia.cityId, user.address, firstBlock).result.expectBool(true)
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block.receipts[0].result.expectOk().expectBool(true);
    block.receipts[0].events.expectSTXTransferEvent(totalAmount, user.address, `${sender.address}.${mia.treasuryV1Contract}`);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    for (let i = 0; i < userIds.length; i++) {
      ...
      block.receipts[i].events.expectSTXTransferEvent(totalCommit, users[i].address, `${sender.address}.${mia.treasuryV1Contract}`);
    }
    ...
    for (let i = 0; i < userIds.length; i++) {
      const expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(firstBlock)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalCommit)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(userIds[i])}}`;
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    ccd005CityData.getCityTreasuryNonce(mia.cityId).result.expectUint(1);
    block.receipts[0].events.expectSTXTransferEvent(totalAmount, sender.address, `${sender.address}.${mia.treasuryV1Contract}`);

    // block.receipts[1].events.expectSTXTransferEvent(totalAmount, sender.address, `${sender.address}.${mia.treasuryV1Contract}`);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    for (let idx = 0; idx < 4; idx++) {
      ...
      block.receipts[idx].events.expectSTXTransferEvent(30, users[idx].address, `${sender.address}.${mia.treasuryV1Contract}`);

      const expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(firstBlock)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount / 4)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(idx + 1)}}`;
    ...
    for (let idx1 = 0; idx1 < entries.length; idx1++) {
      ...
      for (let idx = 0; idx < users.length; idx++) {
        ...
        //dumpMiningData(ccd006CityMining, mia.cityId, (firstBlock + idx1), (idx + 1), miningStatsAt, minerAt);
        checkMiningData(ccd006CityMining, mia.cityId, firstBlock + idx1, idx + 1, miningStatsAt, minerAt);
      }
    }
    ...
    ccd002Treasury.getBalanceStx().result.expectUint(totalAmount);
    ccd006CityMining.getHighValue(mia.cityId, firstBlock).result.expectUint(totalAmount / 3);
    ccd006CityMining.getHighValue(mia.cityId, lastBlock).result.expectUint(totalAmount / 3);
    ccd006CityMining.getHighValue(mia.cityId, lastBlock + 1).result.expectUint(0);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // Check mining events
    const expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(claimHeight)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(1)}}`;
    ...
    const isBlockWinner = ccd006CityMining.isBlockWinner(mia.cityId, user1.address, claimHeight);
    ...
    ccd006CityMining.getBlockWinner(mia.cityId, claimHeight).result.expectNone();
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    chain.mineEmptyBlock(rewardDelay + 1);
    const miningClaimBlock = chain.mineBlock([ccd006CityMining.claimMiningReward(user1, mia.cityName, miningHeight)]);

    // assert
    miningBlock.receipts[0].result.expectOk().expectBool(true);
    // Check mining event
    let expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(miningHeight)}, lastBlock: ${types.uint(miningHeight)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(1)}}`;
    miningBlock.receipts[0].events.expectPrintEvent(`${sender.address}.ccd006-citycoin-mining`, expectedPrintMsg);
    ...
    assertEquals(ccd006CityMining.isBlockWinner(mia.cityId, user1.address, miningHeight).result.expectSome().expectTuple(), expected);
    ccd006CityMining.getBlockWinner(mia.cityId, miningHeight).result.expectSome().expectUint(1);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // act
    const { receipts } = chain.mineBlock([ccd006CityMining.claimMiningReward(sender, mia.cityName, 50)]);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    passProposal(chain, accounts, PROPOSALS.TEST_CCD005_CITY_DATA_001);
    passProposal(chain, accounts, PROPOSALS.TEST_CCD005_CITY_DATA_002);
    const block = chain.mineBlock([ccd006CityMining.claimMiningReward(sender, mia.cityName, 50)]);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    passProposal(chain, accounts, PROPOSALS.TEST_CCD005_CITY_DATA_002);
    chain.mineEmptyBlock(rewardDelay - 1);
    const block = chain.mineBlock([ccd006CityMining.claimMiningReward(sender, mia.cityName, claimBlock.height)]);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const block = chain.mineBlock([ccd006CityMining.claimMiningReward(sender, mia.cityName, claimHeight)]);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    chain.mineEmptyBlock(rewardDelay);
    block = chain.mineBlock([ccd006CityMining.claimMiningReward(user, mia.cityName, claimHeight)]);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // dumpMiningData(ccd006CityMining, mia.cityId, (firstBlock), (1), miningStatsAt, minerAt);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    chain.mineEmptyBlock(rewardDelay);
    const block2 = chain.mineBlock([ccd006CityMining.claimMiningReward(user1, mia.cityName, firstBlock), ccd006CityMining.claimMiningReward(user2, mia.cityName, firstBlock)]);
    ...
    block1.receipts[0].events.expectSTXTransferEvent(10, user1.address, `${sender.address}.${mia.treasuryV1Contract}`);
    block1.receipts[1].events.expectSTXTransferEvent(10, user2.address, `${sender.address}.${mia.treasuryV1Contract}`);

    let expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(firstBlock)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(1)}}`;
    block1.receipts[0].events.expectPrintEvent(`${sender.address}.ccd006-citycoin-mining`, expectedPrintMsg);
    expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(firstBlock)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(2)}}`;
    block1.receipts[1].events.expectPrintEvent(`${sender.address}.ccd006-citycoin-mining`, expectedPrintMsg);

    //dumpMiningData(ccd006CityMining, mia.cityId, (firstBlock), (1), miningStatsAt, minerAt);
    ccd006CityMining.getRewardDelay().result.expectUint(rewardDelay);

    //ccd006CityMining.getBlockWinner(mia.cityId, firstBlock).result.expectUint(2);
    ccd006CityMining.getBlockWinner(mia.cityId, firstBlock).result.expectNone();
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const coinbaseInfo = ccd005CityData.getCityCoinbaseInfo(mia.cityId).result.expectTuple();
    ...
    miningBlock.receipts[0].result.expectOk().expectBool(true);
    miningBlock.receipts[0].events.expectSTXTransferEvent(totalAmount, user1.address, `${sender.address}.${mia.treasuryV1Contract}`);
    miningBlock.receipts[1].result.expectOk().expectBool(true);
    miningBlock.receipts[1].events.expectSTXTransferEvent(totalAmount, user2.address, `${sender.address}.${mia.treasuryV1Contract}`);

    //const miningStatsAt = { amount: 10, claimed: false, miners: 1 };
    //const minerAt = { commit: 10, high: 11, low: 0, winner: false };
    //dumpMiningData(ccd006CityMining, mia.cityId, (firstBlock), (1), miningStatsAt, minerAt);
    //dumpMiningData(ccd006CityMining, mia.cityId, (firstBlock), (2), miningStatsAt, minerAt);
    ...
    let expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(claimHeight)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(1)}}`;
    // console.log(JSON.stringify(miningBlock.receipts[0].events), null, 2);
    miningBlock.receipts[0].events.expectPrintEvent(`${sender.address}.ccd006-citycoin-mining`, expectedPrintMsg);
    expectedPrintMsg = `{cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${mia.treasuryV1Contract}, event: "mining", firstBlock: ${types.uint(claimHeight)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(2)}}`;
    ...
    //ccd006CityMining.getBlockWinner(mia.cityId, firstBlock).result.expectUint(2);
    ccd006CityMining.getBlockWinner(mia.cityId, claimHeight).result.expectSome().expectUint(winner);
  },
});

Clarinet.test({
  name: "ccd006-citycoin-mining-v2: claim-mining-reward() two miners compete and each wins within 10% of half the time",
  fn(chain: Chain, accounts: Map<string, Account>) {
    // arrange
    const sender = accounts.get("deployer")!;
    const user1 = accounts.get("wallet_1")!;
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const claimHeight = miningBlock.height - 1;
    chain.mineEmptyBlock(rewardDelay + 1);
    const miningClaimBlock = chain.mineBlock([ccd006CityMining.claimMiningReward(user1, mia.cityName, claimHeight + 1), ccd006CityMining.claimMiningReward(user1, mia.cityName, claimHeight - 1), ccd006CityMining.claimMiningReward(user1, mia.cityName, claimHeight)]);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // act
    const { result } = ccd006CityMining.getCoinbaseAmount(mia.cityId, 100);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // act
    const { result } = ccd006CityMining.getCoinbaseAmount(mia.cityId, 5);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // const activation = ccd005CityData.getCityActivationDetails(mia.cityId).result.expectSome().expectTuple();
    ...
    for (const threshold of thresholds) {
      const { result } = ccd006CityMining.getCoinbaseAmount(mia.cityId, threshold);
