...
const dumpMiningData = (ccd006CityMining: any, cityId: number, height: number, userId: number) => {
  ...
  height = height + 1;
  // output all mining data
  console.log("mining data check: " + height + " - " + userId + " - " + cityId);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    //console.log(dumpMiningData(ccd006CityMining, miaCityId, firstBlock, userId));
    ccd005CityData.getCityTreasuryNonce(miaCityId).result.expectUint(1);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // simplified pattern when more than 1 or 2
    ...
    for (const [idx, receipt] of block.receipts.entries()) {
      // user ID needs to increase with each pass
      // total needs to be divided by number of miners
      const expectedPrintMsg = `{action: "mining", cityId: u1, cityName: "mia", cityTreasury: ${sender.address}.${miaTreasuryName}, firstBlock: ${types.uint(firstBlock)}, lastBlock: ${types.uint(lastBlock)}, totalAmount: ${types.uint(totalAmount / 4)}, totalBlocks: ${types.uint(totalBlocks)}, userId: ${types.uint(userId + idx)}}`;
      receipt.events.expectPrintEvent(`${sender.address}.ccd006-city-mining`, expectedPrintMsg);
    }
