...
const dumpMiningData = (
  ccd006CityMining: any,
  cityId: number,
  height: number,
  userId: number
) => {
  ...
  console.log(
    "getMiningStatsAtBlock: " +
      ccd006CityMining.getMiningStatsAtBlock(cityId, height).result
  );
  console.log(
    "hasMinedAtBlock: " +
      ccd006CityMining.hasMinedAtBlock(cityId, height, userId).result
  );
  console.log(
    "getMinerAtBlock: " +
      ccd006CityMining.getMinerAtBlock(cityId, height, userId).result
  );
  console.log(
    "getBlockWinner: " + ccd006CityMining.getBlockWinner(cityId, height).result
  );
  //console.log(ccd006CityMining.isBlockWinner(miaCityId, sender.address, (firstBlock + 100) ).result);
  console.log(
    "getHighValue: " + ccd006CityMining.getHighValue(cityId, height).result
  );
  console.log(
    "getCoinbaseAmount: " +
      ccd006CityMining.getCoinbaseAmount(cityId, height).result
  );
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block.receipts[0].result
      .expectErr()
      .expectUint(CCD006CityMining.ErrCode.ERR_CITY_DETAILS_NOT_FOUND);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block.receipts[0].result
      .expectErr()
      .expectUint(CCD003UserRegistry.ErrCode.ERR_UNAUTHORIZED);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block.receipts[0].result
      .expectErr()
      .expectUint(CCD006CityMining.ErrCode.ERR_INVALID_COMMIT_AMOUNTS);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block.receipts[0].result
      .expectErr()
      .expectUint(CCD006CityMining.ErrCode.ERR_INSUFFICIENT_COMMIT);
  },
});
