...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const ccd003CityRegistry = new CCD004CityRegistry(chain, sender, 'ccd004-city-registry');

    // act

    // assert
    ccd003CityRegistry.isDaoOrExtension().result.expectErr().expectUint(CCD004CityRegistry.ErrCode.ERR_UNAUTHORIZED)
  }
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    baseDao.isExtension(EXTENSIONS.CCD003_USER_REGISTRY).result.expectBool(false);
    ...
    ccd003CityRegistry.getUser(1).result.expectSome().expectPrincipal(approver1.address);
    ccd003CityRegistry.getUserId(approver1.address).result.expectSome().expectUint(1);
  }
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    ccd003CityRegistry.getUser(1).result.expectSome().expectPrincipal(approver1.address);
    ccd003CityRegistry.getUserId(approver1.address).result.expectSome().expectUint(1);
    ccd003CityRegistry.getUser(2).result.expectSome().expectPrincipal(approver2.address);
    ccd003CityRegistry.getUserId(approver2.address).result.expectSome().expectUint(2);
    ccd003CityRegistry.getUser(3).result.expectSome().expectPrincipal(approver3.address);
    ccd003CityRegistry.getUserId(approver3.address).result.expectSome().expectUint(3);
  }
});
