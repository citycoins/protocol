...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const receipts = constructAndPassProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_001
    );
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const receipts = constructAndPassProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_002
    );
    ...
    ccd002Treasury
      .getAllowedAsset(EXTERNAL.FT_MIA)
      .result.expectSome()
      .expectBool(true);
    ccd002Treasury
      .getAllowedAsset(EXTERNAL.FT_NYC)
      .result.expectSome()
      .expectBool(false);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    constructAndPassProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_005
    );
    ...
    nft
      ...
      .result.expectOk()
      .expectSome()
      .expectPrincipal(depositor.address);
    const event =
      "{assetContract: " +
      EXTERNAL.NFT_NYC +
      ', caller: ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0, event: "deposit-nft", recipient: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.ccd002-treasury-nyc, sender: ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0, tokenId: u1}';
    ...
    nft
      ...
      .result.expectOk()
      .expectSome()
      .expectPrincipal(EXTENSIONS.CCD002_TREASURY_NYC);
    receipts[0].result.expectOk().expectBool(true);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const receipts = constructAndPassProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_006
    );
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const receipts = passProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_007
    );
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    constructAndPassProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_008
    );
    ccd002Treasury.isAllowed(EXTERNAL.FT_MIA).result.expectBool(true);
    ...
    const receipts = passProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_010
    );
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const receipts = passProposal(
      chain,
      accounts,
      PROPOSALS.TEST_CCD002_TREASURY_009
    );
