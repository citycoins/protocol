...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const ccd002Treasury = new CCD002Treasury(chain, sender, "ccd002-treasury-mia");
    const amount = 1000;
    const event = '{amount: u1000, caller: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM, event: "deposit-stx", recipient: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.ccd002-treasury-mia, sender: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM}';
    ...
    assertEquals(receipts.length, 1);
    receipts[0].result.expectOk().expectBool(true);
    receipts[0].events.expectSTXTransferEvent(amount, sender.address, EXTENSIONS.CCD002_TREASURY_MIA);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const event = '{amount: u1000, assetContract: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.test-ccext-governance-token-mia, caller: ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0, event: "deposit-ft", recipient: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.ccd002-treasury-mia, sender: ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0}';
    ...
    receipts[0].result.expectOk().expectBool(true);
    receipts[0].events.expectFungibleTokenTransferEvent(1000, depositor.address, EXTENSIONS.CCD002_TREASURY_MIA, EXTERNAL.FT_MIA + "::edg-token");
    receipts[0].events.expectPrintEvent(EXTENSIONS.CCD002_TREASURY_MIA, event);
  },
});
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const event = "{assetContract: " + EXTERNAL.NFT_NYC + ', caller: ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0, event: "deposit-nft", recipient: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.ccd002-treasury-nyc, sender: ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0, tokenId: u1}';
    ...
    receipts[0].result.expectOk().expectBool(true);
    receipts[0].events.expectNonFungibleTokenTransferEvent("u1", depositor.address, EXTENSIONS.CCD002_TREASURY_NYC, EXTERNAL.NFT_NYC, "nyc");
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    receipts[3].events.expectSTXTransferEvent(amount2, EXTENSIONS.CCD002_TREASURY_MIA, recipient.address);
...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    assertEquals(receipts.length, 3);
    receipts[2].events.expectNonFungibleTokenTransferEvent("u1", EXTENSIONS.CCD002_TREASURY_NYC, recipient.address, EXTERNAL.NFT_NYC, "nyc");
