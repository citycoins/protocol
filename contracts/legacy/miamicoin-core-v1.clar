...
;; CityCoins Protocol Version 1.0.0
...
(define-data-var cityWallet principal .miamicoin-core-v1)
...
(define-public (register-user (memo (optional (string-utf8 50))))
  (let
    (
      ...
      (initialized (contract-call? .miamicoin-auth is-initialized))
    )
...
(define-public (test-initialize-core (coreContract <coreTrait>))
  (begin
    ...
    (try! (contract-call? .miamicoin-auth test-initialize-contracts coreContract))
    (ok true)
  )
)
